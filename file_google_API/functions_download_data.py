...
def recover_wished_file_name(folder_id, service, from_date, to_date):
    ...
    # Print the name of each file in the list
        if not items:
            ...
            print("Charging content of wished files on the file latest_version_farmbot_data.txt :")
            
            # Clear the previous txt and csv files.
            file_path_txt = desk_path + '/latest_version_farmbot_data.txt'

            with open(file_path_txt, 'wb') as f:
                f.write(b'')

            file_path_csv = desk_path + '/latest_version_farmbot_data.csv'
...
def txt_to_csv():
    ...
    path_csv = desk_path + "/latest_version_farmbot_data.csv"
